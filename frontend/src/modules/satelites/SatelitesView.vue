  ...
  <div class="min-h-full px-10 py-8 text-white">
    <header class="mb-8">
      ...
      <p class="text-sm text-white/40">
        Elementos orbitais TLE de satélites via CelesTrak.
      </p>
    </header>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr
            ...
            class="border-b border-white/[0.05] last:border-0 hover:bg-white/[0.02] cursor-pointer"
            @click="router.push(`/satelites/${sat.NORAD_CAT_ID}`)"
          >
            <td class="px-5 py-3.5 font-medium">{{ sat.OBJECT_NAME }}</td>
            <td class="px-5 py-3.5 text-white/50 font-mono text-xs">
              {{ sat.NORAD_CAT_ID }}
            </td>
            ...
            <td class="px-5 py-3.5 text-white/60">
              {{
                sat.INCLINATION != null
                  ? `${Number(sat.INCLINATION).toFixed(2)}°`
                  : "—"
              }}
            </td>
            <td class="px-5 py-3.5 text-white/50 text-xs">
    ...
    <div v-else-if="!searched" class="text-center py-16">
      <p class="text-white/30 text-sm">
        Digite o nome de um satélite e clique em Buscar.
      </p>
    </div>
  </div>
</template>
