...
const jplUrl = (id) =>
  `https://ssd.jpl.nasa.gov/tools/sbdb_lookup.html#/?sstr=${id}`;
...
<template>
  <div class="min-h-full px-10 py-8 text-white">
    ...
    <template v-else-if="asteroid">
      <div class="flex items-start justify-between gap-6 mb-6">
        <div>
          ...
          <p class="text-sm text-white/50">
            ID: {{ asteroid.id }} · Designação: {{ asteroid.designation }}
          </p>
        </div>
        <a
          :href="jplUrl(asteroid.id)"
          target="_blank"
      ...
      <p class="text-xs text-white/35 mb-8">
        Dados fornecidos pela NASA NeoWs.
      </p>

      <div class="grid grid-cols-4 gap-4 mb-4">
        <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
          <p class="text-[11px] uppercase tracking-widest text-white/40 mb-4">
            Diâmetro Estimado
          </p>
          <dl class="space-y-2.5 text-xs text-white/70">
            <div v-for="unit in diametro" :key="unit.label">
              ...
              {{
                fmt(
                  asteroid.estimated_diameter?.[unit.key]
        ...
        <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
          ...
          <p class="text-xs text-white/40">
            {{ magnitudeLabel(asteroid.absolute_magnitude_h) }}
          </p>
        </div>
        <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
          <p class="text-[11px] uppercase tracking-widest text-white/40 mb-4">
      ...
      <div
        class="rounded-xl border border-white/[0.08] bg-white/[0.03] overflow-hidden mb-6"
      >
        <p
          class="px-5 py-4 border-b border-white/[0.08] text-[11px] uppercase tracking-widest text-white/40"
        >
          Dados de Aproximações Próximas
        </p>
        <div class="overflow-x-auto">
          <table class="w-full text-xs">
            ...
            <tbody>
              <tr
                v-for="(approach, i) in asteroid.close_approach_data"
                :key="i"
              ...
              >
                ...
                <td class="px-4 py-3 text-right text-white/60">
                  {{ fmt(approach.relative_velocity?.kilometers_per_second) }}
                </td>
                <td class="px-4 py-3 text-right text-white/60">
                  {{ fmt(approach.relative_velocity?.kilometers_per_hour) }}
                </td>
                <td class="px-4 py-3 text-right text-white/60">
                  {{ fmt(approach.miss_distance?.astronomical, 10) }}
                </td>
                <td class="px-4 py-3 text-right text-white/60">
                  {{ fmt(approach.miss_distance?.lunar) }}
                </td>
                <td class="px-4 py-3 text-right text-white/60">
                  {{ fmt(approach.miss_distance?.kilometers) }}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
        ...
        <p class="text-sm text-white/55 mb-1">
          Este objeto é um asteroide próximo da Terra (NEO) com aproximações
          registradas ao longo do século XX e XXI.
        </p>
        <p class="text-xs text-white/35">
          Os dados acima são fornecidos pela NASA NeoWs e representam cálculos e
          previsões.
        </p>
        <p class="text-xs text-white/25 mt-4 text-right">
