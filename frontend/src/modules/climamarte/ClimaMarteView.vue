...
const {
  ...
  loading,
  error,
} = useApi({ immediate: true, url: "/insight/weather" });
  ...
  <div class="min-h-full px-10 py-8 text-white">
    <header class="mb-8">
      ...
      <p class="text-sm text-white/40">
        Dados meteorológicos da missão InSight na superfície marciana.
      </p>
    </header>
    ...
    <template v-else-if="latestSol">
      ...
      <div class="grid grid-cols-4 gap-4 mb-6">
        <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
          <p class="text-[11px] uppercase tracking-widest text-white/40 mb-2">
            Temperatura média
          </p>
        ...
        <div class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5">
          ...
          <p class="text-xs text-white/40 mt-1">
            {{ latestSol.data.WD?.most_common?.compass_degrees ?? "—" }}°
            predominante
          </p>
        </div>
      </div>
      ...
      <div
        class="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5 text-xs text-white/50"
      >
        Período UTC: {{ latestSol.data.First_UTC?.split("T")[0] }} —
        {{ latestSol.data.Last_UTC?.split("T")[0] }}
      </div>
    </template>
  </div>
</template>
