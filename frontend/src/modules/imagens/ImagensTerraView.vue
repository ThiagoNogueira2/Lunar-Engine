<script setup>
import { useEarthImagery } from '../../composables/useEarthImagery.js'
  ...
  <div class="min-h-full px-10 py-8 text-white">
    <header class="mb-8">
      ...
      <p class="text-sm text-white/40">
        MODIS Terra (GIBS) — imagem por coordenada via NASA Earthdata.
      </p>
    </header>
    ...
    <form @submit.prevent="handleSearch" class="flex flex-wrap gap-3 mb-6">
      ...
      <input
        ...
        :min="minDate"
        :max="maxDate"
        class="bg-white/[0.04] border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm outline-none focus:border-white/20 [color-scheme:dark]"
      />
      <button
        type="submit"
    ...
    <NasaLoader v-if="loading" message="Gerando imagem..." />
      ...
      <div class="grid grid-cols-3 gap-6">
        <div class="rounded-xl border border-white/[0.05] overflow-hidden">
          <img
            ...
            class="w-full object-cover bg-black"
            loading="lazy"
            decoding="async"
          />
        </div>
        <div
          class="rounded-xl border border-white/[0.05] bg-white/[0.02] p-5 text-sm space-y-3"
        >
          <div>
            <p class="text-[11px] uppercase tracking-widest text-white/40 mb-1">
              Data
            </p>
            <p>{{ result.date ?? "—" }}</p>
          </div>
